...
std::vector<std::vector<KeyPoint>> keypoints_queries;
std::vector<cv::Mat> discriptors_queries;
...
bool object_identification(Mat img_query, Mat img_scene_full, float keep_top_fraction);
...
{ // run in thread
    ...
    {
        bool res = object_identification(i, captured_images, 0.85);
        if (res)
        {
...
/* Function to fill keypoints_queries and descriptors_queries with the extracted values from the query images. Call this once at the beginning.
 */
void query_extraction() {
    int minHessian = 400;
    int nOctaves = 4;
    int nOctaveLayers = 3;
    for (int i = 0; i < query_image.size(); i ++) {
        ...
        detector->detectAndCompute(query_image[i],
                                    Mat(),
                                    keypoints_queries[i],
                                    descriptors_queries[i])
...
bool object_identification(int img_query_index, Mat img_scene_full, float keep_top_fraction)
{

    cv::Mat img_query = query_image[i];
    ...
    */
    ...
    // Given i, we can find these from the globally stored extracted values of img_query
    keypoints_query = keypoints_queries[i];
    descriptors_query = descriptors_queries[i];
